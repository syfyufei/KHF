...
```{R, fig.cap="本文的分析框架", out.width = '100%', fig.pos="h"}
...
```{R, fig.cap="样本的地域分布", out.width = '90%', fig.pos="h", fig.align="center"}
...
t1c1 <- c("知识传授干预与学校课程风格", "政治宣传干预与新闻联播风格", "实践式干预与职业教育知识结合")
t1c2 <- c(0.85, 0.91, 0.96)
table1 <- data.frame(cbind(t1c1, t1c2))
names(table1) <- c("干预类别与模仿类别", "一致性")
...
table2 <- data.frame("干预类别" = c("第一组（知识传授，演示文稿）", "第二组（政治宣传，演示文稿）", "第三组（实践式教育，演示文稿）", "第四组（知识传授，教师呈现与演示文稿）", "第五组（政治宣传，教师呈现与演示文稿）", "第六组（实践式教育，教师呈现与演示文稿）"),
"样本覆盖率" = c("16.14%", "16.5%", "16.77%", "17.04%", "16.75%", "16.81%")) %>% 
  mutate_all(linebreak)

kbl(table2, booktabs = T, align = "c", caption = "思想政治课的六种干预状态") %>%
  kable_styling(latex_options = "hold_position")
...
m1 <- lm(SA ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), data = data)
m2 <- glm(SA_certification ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = data)
m3 <- glm(SA_coercive ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = data)
m4 <- glm(SA_enrich ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = data)
m5 <- glm(SA_rules ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = data)
m6 <- glm(SA_draw ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = data)
m7 <- glm(SA_command ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = data)
m8 <- glm(SA_redistribution ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = data)
...
```{R, fig.cap="政治宣传对学生各类国家能力感知的影响", out.width = '100%', fig.pos="h"}
...
```{R, fig.cap="政治宣传的影响因素", out.width = '100%', fig.pos="h"}
